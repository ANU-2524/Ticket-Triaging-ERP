...
st.set_page_config(page_title="ERP Ticket Triage AI", page_icon="🎫", layout="wide", initial_sidebar_state="expanded")
...
.hero{background:linear-gradient(135deg,#0f1626,#1a2332,#0f1626);border:1px solid #1e3a5f;border-radius:12px;padding:26px 34px;margin-bottom:24px;position:relative;overflow:hidden;}
.hero::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#00d4ff,#7c3aed,transparent);}
.htitle{font-size:24px;font-weight:700;color:#f0f6fc;margin:0;}
.hsub{font-size:13px;color:#8b949e;margin:5px 0 0 0;}
.hbadge{background:#00d4ff15;border:1px solid #00d4ff40;border-radius:20px;padding:4px 14px;font-size:11px;color:#00d4ff;font-weight:600;letter-spacing:1px;text-transform:uppercase;}
.rs{background:#0d1117;border:1px solid #21262d;border-radius:10px;padding:18px 22px;margin-bottom:14px;animation:fsi .3s ease-out;}
@keyframes fsi{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
.rst{font-size:10px;color:#8b949e;text-transform:uppercase;letter-spacing:2px;font-weight:600;margin-bottom:10px;font-family:'IBM Plex Mono',monospace;}
.bh{background:#ff4d4d20;color:#ff6b6b;border:1px solid #ff4d4d40;padding:4px 12px;border-radius:4px;font-size:12px;font-weight:700;}
.bm{background:#ffa94d20;color:#ffc078;border:1px solid #ffa94d40;padding:4px 12px;border-radius:4px;font-size:12px;font-weight:700;}
.bl{background:#69db7c20;color:#8ce99a;border:1px solid #69db7c40;padding:4px 12px;border-radius:4px;font-size:12px;font-weight:700;}
.bi{background:#00d4ff18;color:#00d4ff;border:1px solid #00d4ff35;padding:4px 12px;border-radius:4px;font-size:12px;font-weight:600;}
.rb{background:#161b22;border:1px solid #30363d;border-left:3px solid #00d4ff;border-radius:6px;padding:16px 18px;font-size:14px;color:#c9d1d9;line-height:1.75;font-style:italic;}
.dp{display:inline-block;background:#161b22;border:1px solid #30363d;border-radius:6px;padding:5px 12px;font-size:12px;color:#c9d1d9;margin:3px 3px 3px 0;}
section[data-testid="stSidebar"]{background:#0d1117!important;border-right:1px solid #21262d;}
.stTabs [data-baseweb="tab-list"]{gap:8px;background:transparent;border-bottom:1px solid #21262d;}
.stTabs [data-baseweb="tab"]{background:transparent;border:none;color:#8b949e;font-size:12px;letter-spacing:1px;text-transform:uppercase;font-weight:600;padding:10px 20px;}
.stTabs [aria-selected="true"]{color:#00d4ff!important;border-bottom:2px solid #00d4ff!important;}
.stButton>button{background:linear-gradient(135deg,#00d4ff18,#7c3aed18)!important;border:1px solid #00d4ff60!important;color:#00d4ff!important;font-weight:600!important;letter-spacing:1.5px!important;text-transform:uppercase!important;font-size:12px!important;border-radius:8px!important;width:100%!important;}
textarea{background:#0d1117!important;border-color:#21262d!important;color:#e6edf3!important;}
...
def show_result(r: dict):
    ...
    picon  = {"High":"🔴","Medium":"🟡","Low":"🟢"}.get(priority,"⚪")
    ...
    <div class="rs">
      ...
      <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
        <span class="{pbadge}">{picon} {priority} Priority</span>
        <span class="bi">📂 {category}</span>
        <span class="bi">🔧 {issue_type}</span>
        <span class="bi">💿 {erp_module}</span>
      </div>
    </div>
    <div class="rs">
...
SAMPLES = {
    ...
    "⚙️ SAP – Change Request": "We need to add three new cost centers for our recently acquired subsidiary and map them to the existing GL structure in SAP. Please guide us through the configuration steps or assign a consultant.",
}
...
with tab2:
    if not st.session_state.history:
        ...
        for entry in st.session_state.history:
            r    = entry["result"]
            pico = {"High":"🔴","Medium":"🟡","Low":"🟢"}.get(r.get("priority"),"⚪")
            with st.expander(f'{pico} [{r.get("category")}]  {r.get("summary","")[:65]}...  —  {entry["timestamp"]}'):
                show_result(r)
